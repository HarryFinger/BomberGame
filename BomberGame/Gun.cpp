...
Gun::Gun(const sf::Texture &cannon_texture, const sf::Texture &stand_texture, const sf::Texture &shield_texture)
    : cannon(cannon_texture), stand(stand_texture), shield(shield_texture), original_color(cannon.getColor()),
...
void Gun::update(float delta_time)
{
    ...
    forward_vector =
        tools::NormalizeVector({aim_position.x - cannon.getPosition().x, aim_position.y - cannon.getPosition().y});
